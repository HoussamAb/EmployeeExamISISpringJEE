...
<div class="container">
    ...
    <form:form method="post" action="${pageContext.request.contextPath}/normal/employee/edit" modelAttribute="employees" >
        ...
        <c:if test="${employees.managerEmployee != null}">
            <label class="form-check-label">
                Manager :
            </label>
            <table class="table">
        ...
        <div class="form-check">
            ...
            <form:select class="form-check-label" path="managers" multiple="false">
                <c:forEach items="${managers}" var="nempl">
                    <form:option value="${nempl.id}"> Name: ${nempl.name} | Grade: ${nempl.grade} | Département: ${nempl.departement.name}</form:option>
                </c:forEach>
            </form:select>
            <form:errors path="managers" cssClass="alert-danger" />
        </div>
